...
@PutMapping("/annual/{index}")
    public ResponseEntity annualLeave(@PathVariable int index,@Valid @RequestBody int number,Errors errors)
    {
        ...
        if(number <= Integer.valueOf(employee1.getAnnualLeave()))
        {
            int newNumber = Integer.valueOf(employee1.getAnnualLeave()) - number;
            employee1.setAnnualLeave(String.valueOf(newNumber));
